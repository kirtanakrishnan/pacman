...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    while (iterator < 4):
        ...
        if(goal[iterator] == False):

            total_distance = util.manhattanDistance(problem.corners[iterator], position)
            if (distance == -1):
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    def nearest(coord):
        ...
        near = manhattanDistance(coord, foodGrid.asList()[0])
        nearest_food = foodGrid.asList()[0]

        for food in foodGrid.asList():

            distance = manhattanDistance(coord, food)

            if distance < near:

                nearest_food = food
                near = manhattanDistance(coord, nearest_food)

        return nearest_food

    def furthest(coord):

        far = manhattanDistance(coord, foodGrid.asList()[0])
        furthest_food = foodGrid.asList()[0]
        ...
        for food in foodGrid.asList():
            ...
            if distance > far:
                ...
                furthest_food = food
                far = manhattanDistance(coord, furthest_food)
    ...
    if len(foodGrid.asList()) > 0:
        ...
        near = nearest(position)
        far = furthest(near)

    heuristic =  manhattanDistance(position, near) + manhattanDistance(near, far)
